...
for eta in [0.001, 0.0001]:
    ...
    if classifier.lower() != "multiclass":
        ...
        if classifier.lower() == "perceptron":
            model = perceptron_bin_classifier.Perceptron(eta=eta, iters=20, seed=1)

        elif classifier.lower() == "adaline":
            model = adaline_bin_classifier.Adaline(eta=eta, iters=20, seed=1)

        elif classifier.lower() == "sgd":
            model = sgd_bin_classifier.SGD(eta=eta, iters=20, seed=1)
        ...
        model.print_errors(classifier.lower() + ", eta=" + str(eta), True)
        ...
        for label in set(y_tr):
            ...
            new_model = sgd_bin_classifier.SGD(eta=eta, iters=20, seed=1)
            ...
            new_model.multi_print_errors("Positive class: " + str(label) + ", eta= " + str(eta), True)
